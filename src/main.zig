...
const CPUInfo = osstat.CPUInfo;
const RAMStat = osstat.RAMStat;
const OSInfo = osstat.OSInfo;
...
fn processRequest(request: zap.SimpleRequest) void {
    ...
    const system_info = Infos{
        ...
        .server = ServerInfo{
            ...
            .cpu = CPUInfo{
                .usage = osstat.getCPUPercent(null) orelse 0,
                .arch = trimZerosRight(&uname.machine),
                .model = osstat.parseProcInfo("/proc/cpuinfo", "model name", &cpu_info_buff, null) orelse "Unable to query CPU Name",
            },
            .ram = osstat.getRAMStats(),
            .os = OSInfo{
